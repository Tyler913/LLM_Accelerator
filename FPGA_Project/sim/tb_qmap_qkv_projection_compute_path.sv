...
// End-to-end QMAP-backed Layer 0 Q/K/V projection compute-path test.
//
...
module tb_qmap_qkv_projection_compute_path;
    ...
    localparam int IMAGE_BYTES     = 32'h0000_4000;
    localparam int MEM_WORDS       = IMAGE_BYTES / 4;
    localparam int EXPECTED_WORDS  = 8;
    ...
    logic [31 : 0] expected [0 : EXPECTED_WORDS-1];
    ...
    initial begin
        $readmemh("FPGA_Project/sim/vectors/qmap_qkv_projection_image_words32.hex", mem);
        $readmemh("FPGA_Project/sim/vectors/qmap_qkv_projection_expected_words32.hex", expected);
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (mem_rd_req_valid && mem_rd_req_ready) begin
                active_read      <= 1'b1;
                read_index       <= (mem_rd_req_addr - `QMAP_QKV_BASE_ADDR) >> 2;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (mem_wr_req_valid && mem_wr_req_ready) begin
                active_write <= 1'b1;
                write_index  <= (mem_wr_req_addr - `QMAP_QKV_BASE_ADDR) >> 2;
    ...
    initial begin
        ...
        if (done != 1'b1) begin
            $display("FAIL: timed out waiting for qmap_qkv_projection_compute_path");
            $finish(1);
        end
        ...
        if (expected_index != EXPECTED_WORDS) begin
            $display("FAIL: expected index mismatch actual=%0d expected=%0d", expected_index, EXPECTED_WORDS);
            mismatch_count = mismatch_count + 1;
        end

        if (rows_done != EXPECTED_WORDS) begin
            $display("FAIL: rows_done mismatch actual=%0d expected=%0d", rows_done, EXPECTED_WORDS);
            mismatch_count = mismatch_count + 1;
        end

        if (write_count != EXPECTED_WORDS) begin
            $display("FAIL: write_count mismatch actual=%0d expected=%0d", write_count, EXPECTED_WORDS);
            mismatch_count = mismatch_count + 1;
        end
