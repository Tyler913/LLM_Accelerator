...
module tb_qmap_layer0_full_scheduler;
    ...
    localparam int WRITE_CACHE = 1;
    localparam int WRITE_Q_ROPE = 2;
    localparam int WRITE_ATTN_OUT = 3;
    localparam int WRITE_O_PROJ = 4;
    localparam int WRITE_POST_HIDDEN = 5;
    localparam int WRITE_POST_NORM = 6;
    localparam int WRITE_GATE = 7;
    localparam int WRITE_UP = 8;
    localparam int WRITE_SILU_HIDDEN = 9;
    localparam int WRITE_DOWN = 10;
    localparam int WRITE_LAYER = 11;

    localparam int EXPECTED_NORMAL_RD_REQS = 38055;
    localparam int EXPECTED_NORMAL_RD_WORDS = 1579650;
    localparam int EXPECTED_NORMAL_WR_REQS = 2058;
    localparam int EXPECTED_NORMAL_WR_WORDS = 20480;
    ...
    logic [3 : 0] stage_done_mask;
    logic [3 : 0] stage_error_mask;
    ...
    logic [3 : 0] normal_stage_done_mask;
    logic [3 : 0] normal_stage_error_mask;
    ...
    logic [3 : 0] invalid_stage_done_mask;
    logic [3 : 0] invalid_stage_error_mask;
    logic invalid_error;
    ...
    qmap_layer0_full_scheduler dut (
        .i_clk(clk),
        .i_rst_n(rst_n),
        .i_start(start),
        .i_runtime_context_valid(1'b0),
        .i_runtime_layer_id('0),
        .i_runtime_position('0),
        .i_runtime_kv_cache_base_addr('0),
        .i_residual_base_override_valid(1'b0),
        .i_residual_base_override_addr('0),
        .i_output_base_override_valid(1'b0),
        .i_output_base_override_addr('0),
    ...
    initial begin
        wavefile = "FPGA_Project/wave/qmap_layer0_full_scheduler.vcd";
        if ($value$plusargs("wavefile=%s", wavefile)) begin
        end
        if ($test$plusargs("dumpwaves")) begin
            $dumpfile(wavefile);
            $dumpvars(0, tb_qmap_layer0_full_scheduler);
        end
    end
    ...
    function automatic logic [31 : 0] memory_word(input logic [ADDR_WIDTH-1 : 0] addr);
        ...
        begin
            ...
            if (in_range(addr, `QMAP_ATTN_FRONTEND_BASE_ADDR, FRONT_IMAGE_BYTES)) begin
    ...
    task run_until_done;
        ...
        begin
            ...
            if (done != 1'b1) begin
                $display("FAIL: timed out waiting for qmap_layer0_full_scheduler done");
                $finish(1);
            end
            @(posedge clk);
        end
    ...
    initial begin
        tracefile = "FPGA_Project/sim/qmap_layer0_full_scheduler_trace.csv";
        ...
        if (normal_error) begin
            fail_once("FAIL: normal full scheduler run asserted error");
        end
        if (normal_stage_done_mask != 4'hf || normal_stage_error_mask != 4'h0) begin
            fail_once("FAIL: normal stage masks mismatch");
        end
        ...
        if ((normal_read_bursts != EXPECTED_NORMAL_RD_REQS) ||
            ...
            fail_once("FAIL: full scheduler memory counters mismatch");
        end
        if ((cache_write_accept_count != TOTAL_CACHE_WRITES) ||
        ...
        if (!invalid_error) begin
            fail_once("FAIL: invalid descriptor did not assert error");
        end
        if (invalid_stage_done_mask != 4'h0 || invalid_stage_error_mask != 4'h1) begin
            fail_once("FAIL: invalid run stage masks mismatch");
        end
        if ((invalid_write_reqs != 0) || (invalid_write_words != 0) ||
            (wr_req_accept_count != 0) || (wr_data_accept_count != 0)) begin
            fail_once("FAIL: invalid descriptor produced writes");
        end

        $display("qmap_layer0_full_scheduler chained Layer 0 attention+body test");
        ...
        $display("  stage masks normal     = done 0x%0h error 0x%0h body_done 0x%0h body_error 0x%0h",
                 normal_stage_done_mask, normal_stage_error_mask,
                 normal_body_stage_done_mask, normal_body_stage_error_mask);
        $display("  stage masks invalid    = done 0x%0h error 0x%0h",
                 invalid_stage_done_mask, invalid_stage_error_mask);
        $display("  producer writes        = cache %0d qrope %0d attn %0d oproj %0d post %0d/%0d gate/up %0d/%0d silu %0d down %0d layer %0d",
                 normal_cache_write_accept_count, normal_q_rope_write_accept_count,
        ...
        if ((mismatch_count != 0) || (write_mismatch_count != 0) || (max_abs_diff != 0)) begin
            $display("FAIL: qmap_layer0_full_scheduler found %0d mismatch(es)", mismatch_count);
            $finish(1);
        end

        $display("PASS: qmap_layer0_full_scheduler chained attention front-end through Layer 0 body with exact write-back and error propagation.");
        if (trace_fd != 0) begin
            $fclose(trace_fd);
        end
        $finish;
