...
module tb_mlp_down_proj_stage;
    ...
    logic [CHUNK_BITS-1 : 0] weight_chunk_mem [0:WEIGHT_CHUNK_COUNT-1];
    logic [CHUNK_BITS-1 : 0] scale_chunk_mem [0:SCALE_CHUNK_COUNT-1];
    ...
    initial begin
        wavefile = "FPGA_Project/wave/mlp_down_proj_stage.vcd";
        ...
        $dumpvars(0, dut.projection_core.current_state);
        $dumpvars(0, dut.projection_core.tile_index);
    end
    ...
    function automatic logic out_ready_pattern(input integer cycle, input integer accepted);
        begin
            out_ready_pattern =
                ...
                ((cycle % 7) != 3) &&
                ((cycle % 31) != 11) &&
                !((accepted >= 320) && (accepted <= 380) && ((cycle % 4) != 0)) &&
                !((accepted >= 850) && (accepted <= 910) && ((cycle % 5) == 2));
        end
    endfunction
    ...
    task load_vectors;
        begin
            ...
            $readmemh({vector_dir, "/", prefix, "_weight_chunks4096.hex"}, weight_chunk_mem);
            $readmemh({vector_dir, "/", prefix, "_scale_chunks4096.hex"}, scale_chunk_mem);
    ...
    task pack_inputs;
        begin
            ...
            for (chunk_index = 0; chunk_index < WEIGHT_CHUNK_COUNT; chunk_index = chunk_index + 1) begin
                weight_flat[chunk_index*CHUNK_BITS +: CHUNK_BITS] = weight_chunk_mem[chunk_index];
            end

            for (chunk_index = 0; chunk_index < SCALE_CHUNK_COUNT; chunk_index = chunk_index + 1) begin
                scale_flat[chunk_index*CHUNK_BITS +: CHUNK_BITS] = scale_chunk_mem[chunk_index];
            end
        end
    endtask
            ...
            if (out_stall_active == 1'b1) begin
                if ((out_row !== stalled_out_row) ||
                    ...
                    if (print_count < 32) begin
                        $display("FAIL: MLP down output changed while stalled at cycle %0d", cycle_count);
    ...
    task check_output;
        begin
            if (output_accept_count >= OUT_FEATURES) begin
                if (print_count < 32) begin
                    $display("FAIL: extra MLP down output at cycle %0d", cycle_count);
                    print_count = print_count + 1;
                end
                mismatch_count = mismatch_count + 1;
            end
                ...
                if ((out_row != output_accept_count) ||
                    ...
                    if (print_count < 32) begin
                        $display(
                            "FAIL: MLP down output %0d mismatch row=%0d data=%0d exp=%0d last=%0d",
                            output_accept_count,
                            out_row,
                            out_data,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (rst_n == 1'b0) begin
            ...
            if (trace_fd != 0) begin
                $fwrite(
                    trace_fd,
                    "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,0x%03h,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d\n",
                    cycle_count,
                    start,
                    busy,
                    done,
                    ...
                    saturation,
                    dut.current_state,
                    dut.projection_core.current_state,
                    dut.projection_core.tile_index,
                    out_valid,
                    out_ready,
                    out_valid && out_ready,
                    out_row,
    ...
    initial begin : main_test
        ...
        tracefile = "FPGA_Project/sim/mlp_down_proj_stage_trace.csv";
        ...
        $fwrite(
            trace_fd,
            "cycle,start,busy,done,error,saturation,state,core_state,tile_index,out_valid,out_ready,out_accept,out_row,out_data,out_last,output_count,compute_cycle_count\n"
        );
        ...
        while ((done != 1'b1) && (cycle_count < 180000)) begin
            @(negedge clk);
        end

        if (done != 1'b1) begin
            $display("FAIL: timed out waiting for mlp_down_proj_stage done");
            $finish(1);
        end
        ...
        $display("mlp_down_proj_stage real Layer 0 current-token test");
        ...
        if (saturation) begin
            $display("FAIL: unexpected MLP down output saturation");
            mismatch_count = mismatch_count + 1;
        end
        if (busy) begin
        ...
        if (output_stall_cycles < 300) begin
        ...
        if (compute_cycle_count < 50000) begin
        ...
        if (mismatch_count != 0) begin
            $display("FAIL: %0d mlp_down_proj_stage mismatch(es)", mismatch_count);
            $finish(1);
        end

        $display("PASS: mlp_down_proj_stage matched all 1024 Q12.12 outputs under output backpressure.");
