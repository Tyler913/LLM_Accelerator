...
// Simple smoke test for q4_gemv_tile_1024.
//
// The stimulus uses q_proj rows 0..3 from:
//
...
//
//   FPGA_Project/wave/q4_gemv_tile_1024.vcd
//
// Override paths with:
//
//   vvp FPGA_Project/sim/tb_q4_gemv_tile_1024.vvp \
//     +wavefile=<path> +vectordir=<path>
...
`endif
    ...
    logic [OUT_ROWS*INPUT_SIZE*WEIGHT_WIDTH-1:0] weight_packed_flat;
    logic [OUT_ROWS*GROUP_COUNT*SCALE_WIDTH-1:0] scale_flat;
    logic busy;
    logic done;
    logic [OUT_ROWS*ROW_ACC_WIDTH-1:0] output_flat;

    logic [ACT_WIDTH-1:0] activation_mem [0:INPUT_SIZE-1];
    logic [WEIGHT_WIDTH-1:0] weight_mem [0:OUT_ROWS*INPUT_SIZE-1];
    logic [SCALE_WIDTH-1:0] scale_mem [0:OUT_ROWS*GROUP_COUNT-1];
    logic signed [ROW_ACC_WIDTH-1:0] expected_mem [0:OUT_ROWS-1];
    logic signed [ROW_ACC_WIDTH-1:0] observed_mem [0:OUT_ROWS-1];
    ...
    initial begin
        wavefile = "FPGA_Project/wave/q4_gemv_tile_1024.vcd";
        if ($value$plusargs("wavefile=%s", wavefile)) begin
        end
        $dumpfile(wavefile);
        $dumpvars(0, tb_q4_gemv_tile_1024);
    end

    task load_vectors;
        begin
            $readmemh({vector_dir, "/q4_gemv_tile_1024_activation.hex"}, activation_mem);
            $readmemh({vector_dir, "/q4_gemv_tile_1024_weight.hex"}, weight_mem);
            $readmemh({vector_dir, "/q4_gemv_tile_1024_scale.hex"}, scale_mem);
            $readmemh({vector_dir, "/q4_gemv_tile_1024_expected.hex"}, expected_mem);
        end
    endtask
    ...
    task pack_inputs;
        begin
            ...
            for (row_index = 0; row_index < OUT_ROWS; row_index = row_index + 1) begin
                for (input_index = 0; input_index < INPUT_SIZE; input_index = input_index + 1) begin
    ...
    initial begin
        ...
        #1;
        $display("q4_gemv_tile_1024 q_proj rows 0..3 smoke test");
