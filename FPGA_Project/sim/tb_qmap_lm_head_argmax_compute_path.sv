...
module tb_qmap_lm_head_argmax_compute_path;
    ...
    task load_vectors;
        begin
            $readmemh({vector_dir, "/qmap_lm_head_argmax_image_words32.hex"}, qmap_mem);
            $readmemh({vector_dir, "/qmap_lm_head_argmax_expected_words32.hex"}, expected_words_mem);
    ...
    task run_success(input integer run_id, input integer tile_count);
        ...
        begin
            ...
            wait_cycles = 0;
            while ((done != 1'b1) && (wait_cycles < 900000)) begin
                @(posedge clk);
            ...
            if (done != 1'b1) begin
                $display("FAIL: timed out waiting for qmap LM-head run %0d done", run_id);
                mismatch_count = mismatch_count + 1;
                $finish(1);
            end
    ...
    initial begin
        ...
        if (mem_req_fire_count != ((11 + (MAX_TILES*BURSTS_PER_TILE)) + (11 + (23*BURSTS_PER_TILE)) + 7)) begin
            $display("FAIL: total read request count mismatch actual=%0d", mem_req_fire_count);
            mismatch_count = mismatch_count + 1;
        end
        if (mem_rsp_fire_count != ((1232 + (MAX_TILES*WORDS_PER_TILE)) + (1232 + (23*WORDS_PER_TILE)) + 208)) begin
            $display("FAIL: total read response word count mismatch actual=%0d", mem_rsp_fire_count);
            mismatch_count = mismatch_count + 1;
        end
        ...
        if (mem_wr_req_count != 2 || mem_wr_word_count_total != 6) begin
            $display("FAIL: total write count mismatch reqs=%0d words=%0d", mem_wr_req_count, mem_wr_word_count_total);
            mismatch_count = mismatch_count + 1;
        end
