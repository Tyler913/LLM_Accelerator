...
// QMAP-backed Layer 0 attention output projection wrapper:
//
//   attn_out[2048] + persistent o_proj Q4 weight/scale
//       -> q4_gemv_row_1024(INPUT_SIZE=2048)
//       -> write o_proj_out[1024]
//
// The large Q4 weight and scale tensors are referenced by descriptor base
// address instead of copied into the runtime packet.
module qmap_o_proj_compute_path #(
...
(
    ...
    assign o_mem_rd_req_valid =
        ...
        ((state == S_ACT_REQ) || (state == S_WEIGHT_REQ) || (state == S_SCALE_REQ)) ? 1'b1 :
        1'b0;
    ...
    assign o_mem_rd_rsp_ready =
        ...
        ((state == S_ACT_READ) || (state == S_WEIGHT_READ) || (state == S_SCALE_READ)) ? 1'b1 :
        1'b0;
    ...
    always @* begin
        ...
        shifted = row_sum_q26 >>> SCALE_FRAC;
        ...
        else if (shifted < Q12_12_MIN_EXT) begin
            converted_output_q12_12 = -32'sd8388608;
            converted_saturated = 1'b1;
        end
        else begin
            converted_output_q12_12 = $signed(shifted[31 : 0]);
        end
    end

    always @* begin
        validate_error = 1'b0;
        if ((reader_descriptor_count < `QMAP_O_PROJ_DESCRIPTOR_COUNT) ||
            (reader_descriptor_capacity < `QMAP_O_PROJ_DESCRIPTOR_CAPACITY) ||
            (desc_tensor_id[SLOT_METADATA] != `QMAP_TENSOR_ID_O_PROJ_METADATA) ||
            (desc_tensor_id[SLOT_ACTIVATION] != `QMAP_TENSOR_ID_O_PROJ_ACTIVATION) ||
            (desc_tensor_id[SLOT_WEIGHT] != `QMAP_TENSOR_ID_O_PROJ_WEIGHT) ||
            (desc_tensor_id[SLOT_SCALE] != `QMAP_TENSOR_ID_O_PROJ_SCALE) ||
            (desc_tensor_id[SLOT_OUTPUT] != `QMAP_TENSOR_ID_O_PROJ_OUTPUT) ||
            (desc_tensor_id[SLOT_EXPECTED] != `QMAP_TENSOR_ID_O_PROJ_EXPECTED) ||
            ...
            (desc_scale_tensor_id[SLOT_WEIGHT] != `QMAP_TENSOR_ID_O_PROJ_SCALE) ||
            ...
            (desc_aux0[SLOT_METADATA] != `QMAP_MATRIX_ID_O_PROJ) ||
            (desc_aux0[SLOT_ACTIVATION] != `QMAP_MATRIX_ID_O_PROJ) ||
            (desc_aux0[SLOT_WEIGHT] != `QMAP_MATRIX_ID_O_PROJ) ||
            (desc_aux0[SLOT_SCALE] != `QMAP_MATRIX_ID_O_PROJ) ||
            (desc_aux0[SLOT_OUTPUT] != `QMAP_MATRIX_ID_O_PROJ) ||
                ...
                S_WEIGHT_READ: begin
                    if (i_mem_rd_rsp_valid) begin
                        if (payload_protocol_error) begin
                            ...
                            weight_packed[read_word_index*MEM_DATA_WIDTH +: MEM_DATA_WIDTH] <=
                                i_mem_rd_rsp_data;
                            if (expected_payload_last) begin
                                read_word_index <= 32'd0;
                                state <= S_SCALE_REQ;
